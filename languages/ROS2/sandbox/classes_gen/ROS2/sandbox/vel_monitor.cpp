...
class Monitor : public rclcpp::Node
{
    ...
    : Node("vel_monitor")
    {
        /* Monitor for topic vel */
        vel_statiscics_monitor =
            this->create_subscription<statistics_msgs::msg::MetricsMessage>(
                "vel/statistics", 10, std::bind(&Monitor::vel_statistics_callback, this, _1)
        );
    }
    ...
    // statistics vel callback
    void vel_statistics_callback(
        ...
        if(std::strcmp(stat->metrics_source.c_str(), "message_age") == 0){
            ...
            if (!std::isnan(stat->statistics[DELAY_VALUE_INDEX].data)) {
                ...
                RCLCPP_WARN(this->get_logger(), "[vel] WARNING: unable to "
    ...
    /* subscriptions */
    rclcpp::Subscription<statistics_msgs::msg::MetricsMessage>::SharedPtr vel_statiscics_monitor; 
